...
# LSan-under-ptrace fix: compile the ASan options override once and link into every fuzz binary.
# LSan ptrace-attaches to the process at exit to walk the heap; Mayhem already traces the process
# under ptrace for coverage, so LSan's ptrace call fails → SIGABRT at exit → 0 edges recorded.
# A STRONG __asan_default_options() here overrides the WEAK copy in the ASan runtime and injects
# detect_leaks=0 before any user code runs.  Linking the object (rather than relying on the
# ASAN_OPTIONS env var) is preferred because Mayhem owns the environment.
ASAN_OPTIONS_OBJ="$SRC/mayhem-build/asan_options.o"
...
build_one() {
  ...
  $CXX $CXXFLAGS $DEBUG_FLAGS $LIB_FUZZING_ENGINE "$SRC/mayhem-build/$name.o" \
      "$ASAN_OPTIONS_OBJ" $LIBS $SYS -o "$OUT/$name"
  ...
  $CXX $CXXFLAGS $DEBUG_FLAGS "$SRC/mayhem-build/$name.o" "$STANDALONE_OBJ" \
      "$ASAN_OPTIONS_OBJ" $LIBS $SYS -o "$OUT/$name-standalone"
